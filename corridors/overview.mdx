...
- Skip ids that look like deposit-account ledger rails (e.g. codes/labels such as `NGN_ACCOUNT_USDC` / “NGN account to USDC”) — those need deposit-account `account_id` (and legacy `entity_id` on older integrations), not retail bank transfer
