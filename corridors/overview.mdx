...
- `payment_method.network_id`: from catalog `payment_methods.*.providers[].id` or `GET /partner/banks` `banks[].id`

### Bank

- `payment_method.type`: `bank`
- `payment_method.network_id`: from `GET /partner/banks` `banks[].id` or catalog `payment_methods.bank.providers[].id`
