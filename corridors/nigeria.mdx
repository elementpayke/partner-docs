...
<Warning>
Do **not** pick ids whose codes look like `NGN_ACCOUNT_USDC` / `USDC_NGN_ACCOUNT` (or labels such as “NGN account to USDC” / “USDC to NGN account”). Those are deposit-account ledger rails and require deposit-account account ids (legacy `payment_method.entity_id` + `payment_method.account_id` on older integrations), not retail bank transfer.
</Warning>
