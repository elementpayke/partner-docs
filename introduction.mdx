...
| **Customers** | Vault KYC (`pcus_*`) | Create customer → submit → `approved` → optional `customer_id` on quote; open accounts when `deposit_account=ready`. See [Customers quickstart](/customers/quickstart) |
...
<CardGroup cols={2}>
  ...
  <Card title="Customers quickstart" icon="user" href="/customers/quickstart">
    Vault KYC (`pcus_*`) and optional `customer_id` on quote.
  </Card>
