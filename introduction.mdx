...
Onboard end-customers once into the **customer vault** to get a stable `customer_id` (`pcus_*`) for quotes — then open **EUR/USD IBAN** and **stablecoin** accounts under that same customer. See [Customers quickstart](/customers/quickstart) and [Accounts](/customers/accounts).
