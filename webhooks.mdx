...
Order webhooks (`order.*`) are retried on transient DNS/timeout only (up to 3 attempts, backoff capped at 5s). Non-2xx HTTP responses are **not** retried. Dedupe on `X-Webhook-Id`.

Return **2xx** quickly and do heavy work asynchronously. Always reconcile with a GET rather than relying solely on the webhook.
