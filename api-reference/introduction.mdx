# API reference

Interactive reference for the Element Pay Partner API — discovery, quotes, and order acceptance.

## Included endpoints

| Method | Path |
|--------|------|
| `GET` | `/partner/catalog` |
| `GET` | `/partner/corridors` |
| `GET` | `/partner/payment-methods` |
| `GET` | `/partner/order-requirements` |
| `GET` | `/partner/banks` |
| `GET` | `/partner/rates/indicative` |
| `POST` | `/partner/orders/quote` |
| `POST` | `/partner/orders/{quote_id}/accept` |
| `GET` | `/partner/orders/{order_id}` |



## Customers (vault KYC)

OpenAPI tag: **`partner-customers`**.

| Method | Path |
|--------|------|
| `GET` | `/partner/customers/requirements` |
| `POST` | `/partner/customers` |
| `GET` | `/partner/customers` |
| `GET` | `/partner/customers/{customer_id}` |
| `PATCH` | `/partner/customers/{customer_id}` |
| `POST` | `/partner/customers/{customer_id}/documents` |
| `POST` | `/partner/customers/{customer_id}/submit` |
| `POST` | `/partner/customers/{customer_id}/deposit-account/retry` |

Guide: [Customers quickstart](/customers/quickstart).


## Stablecoin sends

| Method | Path |
|--------|------|
| `GET` | `/partner/customers/{customer_id}/accounts` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/sends/preview` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/sends` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/sends/{send_id}` |

Guide: [Stablecoin sends](/customers/stablecoin-sends).

## Book transfers

| Method | Path |
|--------|------|
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/book-transfers/preview` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/book-transfers` |

Guide: [Book transfers](/customers/book-transfers).

## External bank payouts

| Method | Path |
|--------|------|
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/payouts/preview` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/payouts` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/payouts/{payout_id}` |

Guide: [External bank payouts](/customers/payouts).

## Cards

| Method | Path |
|--------|------|
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/card-charges` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/card-charges/{charge_id}` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/cards` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/cards` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/cards/{card_id}` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/cards/{card_id}/transactions` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/cards/{card_id}/freeze` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/cards/{card_id}/unfreeze` |

Guide: [Cards](/customers/cards).

## Try it

Use **Try it** on any endpoint with your sandbox `X-API-Key`. Requests are sent to the sandbox base URL configured for this site.
