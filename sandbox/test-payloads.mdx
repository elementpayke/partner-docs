...
<Warning>
**Production integrations** must use vault KYC → `customer_id` on quote. Payloads below that use an inline **`customer`** object are for **sandbox QA only** — that path is [deprecated](/orders/quote-and-accept#deprecated-inline-customer) but still supported during migration.
</Warning>
