...
Move **same-currency fiat** between accounts under your partner API key — for example Aly’s EUR account to Khamisi’s EUR account. Source is always a customer-owned fiat `account_id` (`pcus_*`). Destination may be another customer’s (or legacy entity) account id reachable under the **same** key.
...
Staging smoke (Aly → Khamisi):

```bash
export FROM_CUSTOMER="pcus_06a83150db3c"   # Aly
export TO_CUSTOMER="pcus_6bc2cc9626a5"     # Khamisi
...
export ACCOUNT_ID="1"      # Aly EUR (staging smoke)
export TO_ACCOUNT_ID="7"   # Khamisi EUR
```

Sandbox smoke: Acme `pcus_722ec09f39c9` EUR **63** → peer entity/account EUR **16**.
...
Typical success shape (field names may vary slightly; prefer a captured staging body for Try-it):
...
Save `preview_token`. Tokens expire quickly — confirm promptly with a fresh preview if needed.

## 3. Confirm

<Warning>
On **live**, confirm moves **real** fiat balances. Prefer sandbox/staging for full preview → confirm loops unless you intend to transfer funds.
</Warning>
...
Staging smoke: Aly EUR **1** → Khamisi EUR **7**, €0.50 — balances moved (e.g. 0.70→0.20 / 0.50→1.00).
...
Staging smoke (insufficient): Aly EUR **1** available `0.20` — preview `1.20` → **422**; preview `0.20` → **201** (preview only; not confirmed).
