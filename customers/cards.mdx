...
Do not treat a Kenya block on **GBP/EUR** acquiring as a block on **USD** card funding.
...
2. **Acquiring** supports configured fiat currencies (typically USD / GBP / EUR); billing country rules differ by charge currency (see table above). **Issuing** requires a **USD** account.
...
| KE individual opens USD fiat | `422` + `unsupported_region` | Use non-USD fiat; for USD + issuing use a non-KE individual, or a business where Element Pay confirmed USD |
| GBP/EUR charge with non-UK/EU billing | `422` / declined | Use UK/EU billing for GBP/EUR, or charge into a **USD** account where enabled |
