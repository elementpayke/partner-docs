...
<Warning>
**PAN / CVV** are returned only on **issue** (`POST …/cards`) and on explicit **`GET …/cards/{card_id}/credentials`**. List and ordinary GET omit them. Do not log or persist full numbers or CVV.
</Warning>
...
<Steps>
  ...
  <Step title="Issue">
    `POST …/cards` with `type`, `reference`, `cardholder` → card id (PAN/CVV on create)
  </Step>
  <Step title="Show card to the user">
    Call `GET …/cards/{card_id}/credentials` when the UI needs PAN/CVV (do not store them)
  </Step>
  <Step title="Manage">
...
`type` is `virtual` or `disposable`. Disposable cards also require `expiration_month` / `expiration_year`. Optional `spending_controls` apply to virtual cards only. Capture PAN/CVV from the **create** response if shown once at issue time; otherwise use **credentials** (below). Ordinary GET/list omit secrets.

### List / get / transactions

`GET …/cards` **syncs** issued cards from the upstream issuer into Element Pay, then returns the local list (USD fiat accounts only — other currencies **422**). Cards that already exist upstream but were never issued through this API appear with a stable `reference` like `imported-{id}`. Treat `reference` as opaque; use `card_id` for subsequent calls.

List and get return **metadata only** (status, name, last four when known, expiry). Get refreshes status / last four / expiry from upstream when possible — still **no** PAN/CVV.
...
### Reveal credentials (PAN / CVV)

Call this when the frontend needs to show the full card number and CVV (for example after the user opens a “show card” action). Secrets are fetched live and are **not** stored by Element Pay. Prefer this over logging create-response secrets.

```bash
curl -sS "$BASE/partner/customers/$FROM_CUSTOMER/accounts/$USD_ACCOUNT_ID/cards/$CARD_ID/credentials" \
  -H "X-API-Key: $API_KEY" | jq '.'
```

Response includes the usual card metadata plus `number` and `cvv`. Do not write those fields to your database or application logs.
...
3. **PAN/CVV** only on issue create and on `GET …/cards/{card_id}/credentials`; never on list or ordinary get.
4. Reuse stable `reference` values for idempotent creates after timeouts. Synced upstream cards may use an `imported-…` reference — still opaque to partners.
