...
<Steps>
  ...
  <Step title="Wait for approved">
    Poll `GET /partner/customers/{customer_id}` until `status=approved`
  </Step>
  <Step title="Quote (optional)">
    Pass `customer_id` on `POST /partner/orders/quote` instead of a full inline `customer`
  </Step>
  <Step title="Accounts and banking (optional)">
    When `products.deposit_account.status=ready`, see [Accounts and banking](/customers/accounts-and-banking)
  </Step>
</Steps>
...
`approved` alone does not unlock accounts. When `deposit_account` is `ready`, continue to [Accounts and banking](/customers/accounts-and-banking).
...
**Inline `customer` still OK?** Yes. Prefer `customer_id` once the vault case is `approved`.

**Why is my customer `approved` but accounts fail?** Check `products.deposit_account.status` — it must be `ready`. See [Accounts and banking](/customers/accounts-and-banking).

**Where are deposit instructions and stablecoin sends?** Under the same `customer_id` — [Accounts and banking](/customers/accounts-and-banking).

## Next

- [Accounts and banking](/customers/accounts-and-banking) (when `deposit_account=ready`)
