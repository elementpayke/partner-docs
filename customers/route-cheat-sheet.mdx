...
## Signing keys (optional — customer-signed accepts)

Only when Console **Signed accepts** is on for the API key. EVM wallets only. Guide: [Customer-signed accepts](/orders/signed-accepts).

| Method | Path | Notes |
|--------|------|-------|
| `POST` | `/partner/customers/{customer_id}/signing-keys/challenge` | `{ "address": "0x…" }` → `typed_data` + `key_id` |
| `POST` | `/partner/customers/{customer_id}/signing-keys/complete` | `{ "key_id", "signature" }` → active `skey_…` |
| `GET` | `/partner/customers/{customer_id}/signing-keys` | List; active key shows `next_nonce` |
| `POST` | `/partner/customers/{customer_id}/signing-keys/{key_id}/revoke` | Revoke |
...
Same banking gate (`deposit_account=ready`). **Cross-currency** between two accounts owned by the **same** `pcus_*` (for example EUR → NGN before a payout). Binding rate comes from preview — not corridor [`GET /partner/rates/indicative`](/partner/rates/indicative).

| Method | Path | Notes |
|--------|------|-------|
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/conversions/preview` | `to_account_id`, `amount` → `preview_token`, `rate`, `expires_at` |
| `POST` | `/partner/customers/{customer_id}/accounts/{account_id}/conversions` | Confirm + `idempotency_key` (confirm before `expires_at`) |

Guide: [Currency conversions](/customers/conversions).

## External bank payouts

Same banking gate (`deposit_account=ready`). Fiat → outside bank (not another ledger account). Same-currency only. Call discovery before preview. Sandbox full E2E is often unavailable (funding / beneficiary limits); see the guide Warning.

| Method | Path | Notes |
|--------|------|-------|
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/payout-methods` | Schemes + `required_fields` / `require_one_of` |
| `GET` | `/partner/customers/{customer_id}/accounts/{account_id}/payout-banks` | Local bank directory (`bank_code`) for NGN/GHS/CAD |
...
Guide: [External bank payouts](/customers/payouts). Legacy: `/partner/entities/…/payouts` (do not adopt). Not the same as corridor [`GET /partner/banks`](/partner/banks).
...
| Currency conversion | Same banking gate; cross-currency; same `pcus_*`; confirm before short `expires_at` |
