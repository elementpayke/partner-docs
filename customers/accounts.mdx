...
**Optional banking product** — EUR/USD IBAN, stablecoin ledger, sends, book transfers, payouts, and cards.
...
Use each `account_id` for [deposit instructions](/customers/deposit-instructions), [stablecoin sends](/customers/stablecoin-sends), book transfers / payouts / cards. Corridor [quote → accept](/orders/quote-and-accept) uses **`customer_id` only** — no account open required.
