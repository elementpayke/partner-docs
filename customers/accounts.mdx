...
**Optional banking product** — EUR/USD IBAN, stablecoin ledger, sends, book transfers, currency conversions, payouts, and cards.

<Warning>
**Prerequisite — KYC (required for everyone):** complete [Customer KYC quickstart](/customers/quickstart) until `status` is `approved`. Banking does **not** replace KYC.

**Prerequisite — banking gate:** `products.deposit_account.status` must be **`ready`** before account open / list / sends. KYC approval alone is **not** enough for this section.
</Warning>

Give each vault customer **bankable and on-chain balances** under one `customer_id` (`pcus_*`):

- **Fiat accounts** — e.g. **EUR** (IBAN) and **USD** (account / routing or SWIFT when returned)
- **Stablecoin accounts** — e.g. **USDC** / **USDT** on **Base** or **Polygon**

Use each `account_id` for [deposit instructions](/customers/deposit-instructions), [stablecoin sends](/customers/stablecoin-sends), [book transfers](/customers/book-transfers), [currency conversions](/customers/conversions), [payouts](/customers/payouts), and [cards](/customers/cards). Corridor [quote → accept](/orders/quote-and-accept) uses **`customer_id` only** — no account open required.
...
Production: `https://api.elementpay.net/api/v1` with `is_live_…` keys.
...
| Gate | Field | Unlocks |
|------|-------|---------|
| **Quote identity** | `status` is `approved` | `customer_id` on quote |
...
<Steps>
  <Step title="Confirm banking ready">
    Prefer `customer.deposit_account.updated` ([Webhooks](/webhooks)); or poll `GET /partner/customers/{customer_id}` until `data.products.deposit_account.status` is `ready`
  </Step>
