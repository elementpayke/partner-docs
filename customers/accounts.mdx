...
Open and read customer-owned account rails (`pcus_*` → `account_id`). Use the `account_id` for [deposit instructions](/customers/deposit-instructions), [stablecoin sends](/customers/stablecoin-sends), and later book transfers / payouts / cards.
...
`network` is required: `Base`, `Polygon`, or `Stellar`. Currency: `USDC` or `USDT` (USDT is not available on Stellar).
